...
/* ─── Fabric Category / Icon Mapper ────────────────────────── */
function materialDetails(name: string): { icon: string; bg: string; text: string } {
  const n = name.toLowerCase();
  if (n.includes('silk') || n.includes('satin')) {
    return { icon: '✨', bg: 'bg-amber-50 border-amber-200', text: 'text-amber-900' };
  }
  if (n.includes('cotton') || n.includes('linen')) {
    return { icon: '🌿', bg: 'bg-emerald-50 border-emerald-200', text: 'text-emerald-900' };
  }
  if (n.includes('wool') || n.includes('cashmere') || n.includes('tweed')) {
    return { icon: '🧶', bg: 'bg-rose-50 border-rose-200', text: 'text-rose-900' };
  }
  if (n.includes('leather') || n.includes('suede')) {
    return { icon: '💼', bg: 'bg-orange-50 border-orange-200', text: 'text-orange-900' };
  }
  if (n.includes('velvet') || n.includes('brocade')) {
    return { icon: '👑', bg: 'bg-purple-50 border-purple-200', text: 'text-purple-900' };
  }
  if (n.includes('denim') || n.includes('jean')) {
...
function MaterialDrawer({
  ...
  function field(key: keyof Material) {
    return (material as Record<string, unknown>)[key] as string ?? '';
  }
    ...
    <>
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-white/50 mt-0.5">
                {isNew ? 'Create a new fabric material' : field('name')}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            {showDeleteConfirm && (
              <div className="flex items-center gap-2 rounded-lg bg-red-900/40 px-3 py-1.5">
                <span className="text-xs text-red-200">Delete material?</span>
                <button
                  onClick={onDelete}
                  disabled={deleting}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
            ...
            <div className="rounded-xl border border-[#7A021D]/20 bg-[#FDF8F4] p-4 space-y-2 text-xs">
              <div className="flex items-center justify-between font-semibold text-[#7A021D]">
                <span className="flex items-center gap-1.5">
                  <span>💡</span> Material Help & Guidelines
                </span>
              ...
              <ul className="space-y-1.5 text-neutral-600 leading-relaxed list-disc pl-4">
                <li><strong className="text-[#2C0505]">Material Name:</strong> Enter standard fabric names (e.g. <em>Mulberry Silk, Italian Leather, Organic Cotton, Cashmere</em>).</li>
                <li><strong className="text-[#2C0505]">Automatic Icons:</strong> Fabrics containing keywords like <em>Silk (✨), Cotton (🌿), Leather (💼), Wool (🧶), Velvet (👑), Denim (👖)</em> automatically receive themed visual badges.</li>
                <li><strong className="text-[#2C0505]">Slug:</strong> A URL-friendly identifier. Auto-generated from the name if left blank.</li>
                <li><strong className="text-[#2C0505]">Sort Order:</strong> Determines priority in product drop-down selectors (lower numbers appear first).</li>
              </ul>
            </div>
          )}
          <div>
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. mulberry-silk (auto-generated if empty)"
            />
          </div>
          ...
          <div>
            ...
            <input
              type="number"
              value={(material as Material).sort_order ?? 0}
        ...
        <div className="shrink-0 flex items-center justify-end gap-3 border-t border-neutral-200 bg-white px-6 py-4">
          ...
          <button
            onClick={onSave}
            disabled={saving || !((material as Material).name?.trim())}
...
export default function MaterialsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { items } = await Materials.list(q || undefined);
      setMaterials(items);
    } finally {
      setLoading(false);
    }
  ...
  async function handleSave() {
    ...
    try {
      if (editingId) {
        await Materials.update(editingId, draft);
      } else {
        await Materials.create(draft);
      }
      invalidateTaxonomyCache();
      await load();
      closeModal();
      showToast('success', editingId ? 'Material updated' : 'Material created');
    } catch (e) {
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      await Materials.remove(editingId);
      invalidateTaxonomyCache();
      await load();
      closeModal();
  ...
  const filteredMaterials = useMemo(() => {
    ...
    return materials.filter(
      (m) => m.name.toLowerCase().includes(lq) || m.slug.toLowerCase().includes(lq),
    );
  }, [materials, q]);
    ...
    <>
      <div className="min-h-full bg-neutral-50 p-6">
        ...
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-[#2C0505]">Materials</h1>
            <p className="mt-0.5 text-sm text-neutral-500">
              {loading ? 'Loading…' : `${materials.length} material type${materials.length !== 1 ? 's' : ''}`}
            </p>
          </div>
          <button
            onClick={openAdd}
        ...
        {!loading && filteredMaterials.length === 0 && (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="text-sm font-medium text-neutral-700">No materials found</p>
            <p className="mt-1 text-xs text-neutral-400">Add fabric materials to assign to products</p>
            <button
              onClick={openAdd}
        ...
        {/* ══════════════════════════════════════
            GRID VIEW
        ══════════════════════════════════════ */}
        {!loading && filteredMaterials.length > 0 && view === 'grid' && (
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
            {filteredMaterials.map((mat) => {
              const details = materialDetails(mat.name);
        ...
        {/* ══════════════════════════════════════
            LIST VIEW
        ══════════════════════════════════════ */}
        {!loading && filteredMaterials.length > 0 && view === 'list' && (
