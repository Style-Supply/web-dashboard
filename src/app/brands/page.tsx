...
async function uploadBrandSizeChart(file: File): Promise<string> {
  ...
  const res = await fetch(`${API_BASE}/api/admin/brands/size-chart-upload`, {
    method: 'POST',
    headers,
    body: formData,
  });
...
/* ─── brand size chart uploader component ─────────────────── */
interface BrandSizeChartUploaderProps {
  value: string | null;
  onChange: (url: string | null) => void;
}

function BrandSizeChartUploader({ value, onChange }: BrandSizeChartUploaderProps) {
  const [uploading, setUploading] = useState(false);
  const [dragOver, setDragOver] = useState(false);
  const inputRef = useRef<HTMLInputElement>(null);
  const { showToast } = useToast();
  ...
  async function handleFile(file: File) {
    ...
    setUploading(true);
    try {
      const url = await uploadBrandSizeChart(file);
      onChange(url);
      showToast('success', 'Default size chart uploaded');
    } catch (e) {
      showToast('error', e instanceof Error ? e.message : 'Upload failed');
    } finally {
      setUploading(false);
    ...
    <div>
      <div className="flex items-center justify-between mb-1.5">
        <label className="block text-xs font-medium text-neutral-500">
          Default Size Chart Image (Optional)
        </label>
        <span className="text-[11px] text-neutral-400">
          Used for all brand products without a custom chart
        </span>
      </div>
        ...
        >
          {uploading ? (
            <div className="flex flex-col items-center gap-2">
              <span className="h-5 w-5 animate-spin rounded-full border-2 border-[#7A021D] border-t-transparent" />
              <p className="text-xs font-medium text-[#7A021D]">Uploading default size chart…</p>
            </div>
          ) : (
            <div className="flex flex-col items-center gap-1.5 text-center">
              ...
              <p className="text-xs font-medium text-neutral-700">
                {dragOver ? 'Drop image here' : 'Click or drag to upload default brand size chart'}
              </p>
    ...
    <>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <BrandSizeChartUploader
            value={(brand as Brand).size_chart_url ?? null}
